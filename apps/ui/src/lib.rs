...
/// Starts the application: window, runtime, monitoring.
///
/// Public because there are two ways in. On a desktop the binary calls it from `main`;
/// on Android there is no `main` at all — the system loads this library and calls
/// [`android_main`], which sets up the platform and then calls this.
...
///
/// There is no `main` on Android: the system loads this library and calls this function,
/// which is why the whole application had to become one. `no_mangle` is what makes the
/// symbol findable by that name, and the crate's `deny(unsafe_code)` is lifted for
/// exactly this item — the attribute is unsafe to *apply*, not unsafe to run.
#[cfg(target_os = "android")]
#[allow(unsafe_code)]
#[unsafe(no_mangle)]
fn android_main(app: slint::android::AndroidApp) {
    // The data directory has to be settled before anything reads it. `dirs::data_dir()`
    // has no answer on Android, and the fallback — a directory beside the executable —
    // is not writable there. The application already supports an override, so this uses
    // it rather than teaching `AppPaths` about a platform it otherwise never sees.
    if let Some(path) = app.internal_data_path() {
        // SAFETY-adjacent note, not an unsafe block: this runs before any thread is
        // spawned, which is the condition `set_var` asks for.
        unsafe { std::env::set_var(vds_composition::paths::DATA_DIR_ENV, path) };
    }

    if let Err(error) = slint::android::init(app) {
        // No window yet, so there is nowhere to show this but the log.
        tracing::error!(%error, "could not start the Android backend");
        return;
    }

    if let Err(error) = run() {
        tracing::error!(%error, "the application stopped");
    }
}
